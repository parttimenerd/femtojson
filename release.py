#!/usr/bin/env python3
"""
Bump version and release femtojson.
...
class VersionBumper:
    ...
    def update_readme(self, old_version: str, new_version: str):
        ...
        content, count = re.subn(
            r"(<artifactId>femtojson</artifactId>\s*\n\s*<version>)([\d.]+)(</version>)",
...
"""
        ...
        try:
            jar_path = self.project_root / 'target' / f'femtojson-{version}.jar'
...
def main():
    parser = argparse.ArgumentParser(
        description='Bump version and release femtojson',
...
Note: CHANGELOG.md must have content under [Unreleased] section before releasing.
    ...
    if args.dry_run:
        ...
        if do_github_release:
            print(f"  • gh release create v{new_version} (with CHANGELOG entry + femtojson-{new_version}.jar)")
    ...
    print("\nArtifacts:")
    print(f"  • target/femtojson-{new_version}.jar")
    print(f"  • target/femtojson-{new_version}-sources.jar")
    print(f"  • target/femtojson-{new_version}-javadoc.jar")
